...
def extract_all_yahoo_financials(ticker_symbol):
    ...
    df["Date"] = income.columns[0].date() if not income.empty else None  # ✅ Add date from quarterly financials
    return df
...
if __name__ == "__main__":
    ...
    for ticker in ticker_list:
        ...
        df.to_csv(f"yahoo_financial_features_{ticker}.csv", index=False)
