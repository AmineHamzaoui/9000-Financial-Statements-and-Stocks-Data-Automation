...
main_df = pd.read_csv(
    'C:/Users/nss_1/Pictures/Automated_BSA_pipeline/final_features_output.csv', sep=';', low_memory=False)    
# Example usage
if __name__ == "__main__":
    ticker_list = get_ticker_list()

    # Make sure Date column is datetime in main_df
    ...
    for ticker in ticker_list:
        ...
        sub_df = main_df[main_df["Ticker"] == ticker].copy().sort_values("Date")
        if sub_df.empty:
        ...
        fin_wide = df.pivot(index="Date", columns="Metric", values="Value").reset_index()
        ...
        # Add only new columns to main_df
        new_cols = fin_wide.columns.drop("Date")
        for col in new_cols:
            if col in sub_df.columns:
                merged.drop(columns=[col], inplace=True)

        # Update main_df with merged new columns
        main_df.loc[merged.index, merged.columns.difference(sub_df.columns)] = merged[merged.columns.difference(sub_df.columns)]

        main_df.to_csv("updated_main_df.csv", index=False, sep=";")
        print("📁 Saved updated main_df to 'updated_main_df.csv'")
