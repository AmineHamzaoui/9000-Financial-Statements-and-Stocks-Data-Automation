...
def extract_all_yahoo_financials(ticker_symbol):
    ...
    capex = -safe_get(cashflow, "Capital Expenditures") if safe_get(cashflow, "Capital Expenditures") else None
    ...
    features = {
        ...
        "Total Long-Term Assets": None,  # Not directly available
        "Total Assets": assets,
    ...
    return pd.DataFrame(features.items(), columns=["Metric", "Value"])

# Example usage
df = extract_all_yahoo_financials("AAPL")
df.to_csv("yahoo_financial_features_AAPL.csv", index=False)
print(df)
