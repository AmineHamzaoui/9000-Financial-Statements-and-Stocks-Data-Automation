...
def append_all_tickers_once(tickers, file_path):
    ...
    # Filter existing_df to today's date only once
    existing_today = existing_df[existing_df['Date'].dt.strftime(
        '%Y-%m-%d') == today_str]
    tickers_already_present = set(existing_today['Ticker'].unique())

    all_new_data = []

    for ticker in tickers:
        if ticker in tickers_already_present:
            print(f"⏭️ Skipping {ticker}: already present for today.")
            continue
    ...
    print("✅ All new data appended and saved.")
