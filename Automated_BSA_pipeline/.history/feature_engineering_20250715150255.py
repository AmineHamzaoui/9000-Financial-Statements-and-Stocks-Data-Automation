...
# ----------------- Config -----------------
INPUT_FILE = 'C:/Users/nss_1/Pictures/Automated_BSA_pipeline/Test_dataset.csv'
FINAL_OUTPUT = 'final_features_output_updated.csv'
...
def load_new_data():
    ...
    if os.path.exists(FINAL_OUTPUT):
        ...
        merged = df.merge(prev[['Ticker_x', 'date_x']], on=[
                          'Ticker_x', 'date_x'], how='left', indicator=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Drop overlapping financial ratio columns if they already exist
        ratio_cols = [
            'P/S', 'P/B', 'Price/Cash', 'Price/Free Cash Flow', 'Gross Margin (%)',
            'Operating Margin (%)', 'Net Profit Margin (%)', 'ROA (%)', 'ROE (%)',
            'ROI (%)', 'P/E', 'EV', 'EV/EBITDA', 'EV/Sales', 'dividend_growth'
        ]
        existing_cols = [col for col in ratio_cols if col in final_df.columns]
        if existing_cols:
    ...
    try:
        ...
        # Safely join YTD_Base_Close avoiding column overlap
        ytd_base = final_df.groupby(['Ticker_x', 'Year'])['Close'].first().rename('YTD_Base_Close')
        if 'YTD_Base_Close' in final_df.columns:
            final_df.drop(columns=['YTD_Base_Close'], inplace=True)
        final_df = final_df.join(ytd_base, on=['Ticker_x', 'Year'])

        # Drop existing performance columns if they exist
        perf_cols = ['Performance YTD', 'Performance 1Y', 'Performance 3Y', 'Performance 5Y']
        existing_perf = [col for col in perf_cols if col in final_df.columns]
        if existing_perf:
            final_df.drop(columns=existing_perf, inplace=True)
    ...
    try:
        if os.path.exists(FINAL_OUTPUT):
            existing_df = pd.read_csv(
                FINAL_OUTPUT, parse_dates=['date_x'], sep=';')
            combined_df = pd.concat([existing_df, final_df], ignore_index=True)
            combined_df.drop_duplicates(
                subset=['Ticker_x', 'date_x'], keep='last', inplace=True)
        else:
            combined_df = final_df

        validate_features(
            combined_df, ['P/S', 'P/B', 'EV/EBITDA', 'EV/Sales', 'P/E'])
