...
if __name__ == "__main__":
    ...
    final_df['EPS Growth TTM'] = final_df.groupby('Ticker')['EPS - Earnings Per Share'].pct_change(periods=252) * 100
    final_df['EPS Growth Past 3 Years'] = final_df.groupby('Ticker')['EPS - Earnings Per Share'].pct_change(periods=756) * 100
    final_df['EPS Growth Past 5 Years'] = final_df.groupby('Ticker')['EPS - Earnings Per Share'].pct_change(periods=1260) * 100
    final_df['Sales Growth QoQ'] = final_df.groupby('Ticker')['Revenue'].pct_change(periods=1) * 100
    final_df['Sales Growth TTM'] = final_df.groupby('Ticker')['Revenue'].pct_change(periods=252) * 100
    final_df['Sales Growth Past 3 Years'] = final_df.groupby('Ticker')['Revenue'].pct_change(periods=756) * 100
    final_df['Sales Growth Past 5 Years'] = final_df.groupby('Ticker')['Revenue'].pct_change(periods=1260) * 100
    final_df['ROIC'] = final_df['EBIT'] / (final_df['Total Assets'] - final_df['Total Current Liabilities'])
    final_df['Quick Ratio'] = (final_df['Total Current Assets'] - final_df['Inventory']) / final_df['Total Current Liabilities']
    ...
    first_close = final_df.groupby(['Ticker', 'Year'])['Close'].first().rename('YTD_Base_Close')

    # Join back to original data (matches on Ticker and Year)
    final_df = final_df.join(first_close, on=['Ticker', 'Year'])

    # Compute YTD Return
    final_df['YTD Return'] = (final_df['Close'] / final_df['YTD_Base_Close'] - 1) * 100
    final_df['1Y Return'] = final_df.groupby('Ticker')['Close'].pct_change(periods=252) * 100
    final_df['3Y Return'] = final_df.groupby('Ticker')['Close'].pct_change(periods=756) * 100
    final_df['5Y Return'] = final_df.groupby('Ticker')['Close'].pct_change(periods=1260) * 100
    final_df['Performance 1Y'] = final_df.groupby('Ticker')['Close'].pct_change(periods=252) * 100
    final_df['Performance 3Y'] = final_df.groupby('Ticker')['Close'].pct_change(periods=252 * 3) * 100
final_df['Performance 5Y'] = final_df.groupby('Ticker')['Close'].pct_change(periods=252 * 5) * 100
