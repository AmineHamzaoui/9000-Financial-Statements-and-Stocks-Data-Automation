        ...
        for i, chunk in enumerate(tqdm(reader, desc="🔄 Processing chunks")):
            try:
                ...
                features = features.drop(columns=[
                                         col for col in features.columns if col in original_cols.columns and col not in ['Ticker_x', 'date_x']])
                processed = pd.concat([original_cols.reset_index(
                    drop=True), features.reset_index(drop=True)], axis=1)
