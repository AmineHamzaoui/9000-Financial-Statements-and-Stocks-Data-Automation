...
def get_eps_growth_dataframe():
    ...
    for t in tickers:
        try:
            ...
            eps_growth_5y = info.get("earningsGrowth")  # often 5Y CAGR
            eps_qoq = info.get("earningsQuarterlyGrowth")  # QoQ

            data.append({
                "Ticker": t,
                "EPS Growth [5Y] (%)": round(eps_growth_5y * 100, 2) if eps_growth_5y else None,
                "EPS Growth QoQ (%)": round(eps_qoq * 100, 2) if eps_qoq else None
            })
...
# Example usage
if __name__ == "__main__":
    df_eps_growth = get_eps_growth_dataframe()
    print(df_eps_growth)
"""
ticker.info.get("revenueGrowth")  # Often YoY
