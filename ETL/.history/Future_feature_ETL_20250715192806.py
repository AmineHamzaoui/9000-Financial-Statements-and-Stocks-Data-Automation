...
if __name__ == "__main__":
    ...
    # Step 2: Load first chunk separately to store original columns
    original_df = pd.read_csv(INPUT_FILE, sep=';', nrows=1000)
    ...
    try:
        ...
        for i, chunk in enumerate(tqdm(reader, desc="🔄 Processing chunks")):
            try:
                original_cols = chunk.copy()
                features = process_chunk(chunk)

                # Merge to preserve original columns
                features = features.drop(columns=[col for col in features.columns if col in original_cols.columns and col not in ['Ticker_x', 'date_x']])
                processed = pd.concat([original_cols.reset_index(drop=True), features.reset_index(drop=True)], axis=1)
    ...
    try:
        ...
        base_df = base_df.merge(yahoo_df, on='Ticker_x', how='left')
    except Exception as e:
    ...
    # Step 8: Validate + Save with correct column order
    try:
        # Ensure we keep the original column order first, then add new ones
        final_cols = original_columns + \
            [col for col in final_df.columns if col not in original_columns]
        final_df = final_df[[
            col for col in final_cols if col in final_df.columns]]
