...
financial_data = financial_data.sort_values(by=['Ticker', 'date']).reset_index(drop=True)

# === Step 8: Final monotonicity check ===
merged_check = merged_df.groupby("Ticker")['Date'].apply(lambda x: x.is_monotonic_increasing).all()
finance_check = financial_data.groupby("Ticker")['date'].apply(lambda x: x.is_monotonic_increasing).all()
...
if not (merged_check and finance_check):
    raise ValueError("❌ Sorting failed: some tickers have non-increasing dates.")
