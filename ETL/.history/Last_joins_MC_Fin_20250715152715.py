...
Macro_YF = pd.read_csv("D:/ETL/Macro_YF_1.csv", sep=";", on_bad_lines='warn', low_memory=False)
...
financial_data['Ticker'] = financial_data['Ticker'].astype(str).str.upper().str.strip()

# === Step 5: Merge-asof by Ticker ===
results = []
common_tickers = set(merged_df['Ticker']).intersection(set(financial_data['Ticker']))
...
# Optional: forward fill per Ticker
df_merged.update(df_merged.groupby('Ticker').ffill())

# === Step 7: Load Sector/Industry metadata ===
df_meta = pd.read_csv("D:/ETL/tickers_sector_industry.csv", sep=';')  # Path to your metadata
df_meta['Ticker'] = df_meta['Ticker'].astype(str).str.upper().str.strip()

# === Step 8: Merge metadata using Ticker_x ===
...
# Drop duplicate Ticker column after merge
df_merged.drop(columns=['Ticker'], inplace=True)

# Forward fill Sector and Industry by Ticker_x
df_merged[['Sector', 'Industry']] = df_merged.groupby('Ticker_x')[['Sector', 'Industry']].ffill()

# === Step 9: Export final DataFrame ===
df_merged.to_csv("stock_with_fundamentals_with_sector.csv", index=False, sep=';')
print("✅ Final DataFrame saved as 'stock_with_fundamentals_with_sector.csv'")
