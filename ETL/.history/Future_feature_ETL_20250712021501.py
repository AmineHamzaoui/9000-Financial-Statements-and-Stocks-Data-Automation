...
if __name__ == "__main__":
    ...
    try:
        ...
        final_df['Dividends'] = pd.to_numeric(final_df['Dividends'], errors='coerce').fillna(0)
    ...
    try:
        final_df['EPS - Earnings Per Share'] = pd.to_numeric(final_df['EPS - Earnings Per Share'], errors='coerce')
        final_df['Revenue'] = pd.to_numeric(final_df['Revenue'], errors='coerce')
        final_df['EBIT'] = pd.to_numeric(final_df['EBIT'], errors='coerce')
        final_df['Total Assets'] = pd.to_numeric(final_df['Total Assets'], errors='coerce')
        final_df['Total Current Liabilities'] = pd.to_numeric(final_df['Total Current Liabilities'], errors='coerce')
        final_df['Inventory'] = pd.to_numeric(final_df['Inventory'], errors='coerce')
        final_df['Total Current Assets'] = pd.to_numeric(final_df['Total Current Assets'], errors='coerce')

        final_df['EPS Growth TTM'] = final_df.groupby('Ticker_x')['EPS - Earnings Per Share'].pct_change(periods=252) * 100
        final_df['EPS Growth Past 3 Years'] = final_df.groupby('Ticker_x')['EPS - Earnings Per Share'].pct_change(periods=756) * 100
        final_df['EPS Growth Past 5 Years'] = final_df.groupby('Ticker_x')['EPS - Earnings Per Share'].pct_change(periods=1260) * 100
        final_df['Sales Growth QoQ'] = final_df.groupby('Ticker_x')['Revenue'].pct_change(periods=1) * 100
        final_df['Sales Growth TTM'] = final_df.groupby('Ticker_x')['Revenue'].pct_change(periods=252) * 100
        final_df['Sales Growth Past 3 Years'] = final_df.groupby('Ticker_x')['Revenue'].pct_change(periods=756) * 100
        final_df['Sales Growth Past 5 Years'] = final_df.groupby('Ticker_x')['Revenue'].pct_change(periods=1260) * 100
    ...
    try:
        validate_features(final_df, ['P/S', 'P/B', 'EV/EBITDA', 'EV/Sales', 'P/E'])
