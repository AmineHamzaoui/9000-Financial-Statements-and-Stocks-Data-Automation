...
df_meta = pd.read_csv("D:/ETL/tickers_sector_industry.csv",
                      sep=',')  # Path to your metadata
df_meta['Ticker'] = df_meta['Ticker'].astype(str).str.upper().str.strip()

# === Step 8: Merge metadata using Ticker_x ===
# === Step 8: Merge only Sector and Industry using Ticker ===
df_meta = pd.read_csv("D:/ETL/tickers_sector_industry.csv", sep=',')
df_meta['Ticker'] = df_meta['Ticker'].astype(str).str.upper().str.strip()

# Just bring in Sector and Industry — no duplicate Ticker column
df_merged = df_merged.merge(
    df_meta[['Ticker', 'Sector', 'Industry']],
    how='left',
    left_on='Ticker',
    right_on='Ticker'
)

# Forward fill Sector and Industry by Ticker
df_merged[['Sector', 'Industry']] = df_merged.groupby('Ticker')[['Sector', 'Industry']].ffill()
# === Step 9: Export final DataFrame ===
df_merged.to_csv("stock_with_fundamentals_with_sector.csv",
                 index=False, sep=';')
