...
# === Step 5.5: Debug unsorted Ticker groups (merged_df) ===
for ticker, group in merged_df.groupby("Ticker"):
    if not group['Date'].is_monotonic_increasing:
        print(f"🔍 Ticker {ticker} has unsorted dates:")
        print(group[['Date']].head(10))
        break  # show only the first problematic one

# === Step 5.6: Safety checks before merge_asof ===
