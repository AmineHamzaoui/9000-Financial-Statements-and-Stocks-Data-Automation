...
folders = [
    r"D:\ETL\balance-sheet",
    r"D:\ETL\cash-flow-statement",
    r"D:\ETL\financial-ratios",
    r"D:\ETL\income-statement"
]
...
for folder in folders:
    for filepath in glob.glob(os.path.join(folder, "*.json")):
        ...
        for item in full_data:
            ...
            soup_name = BeautifulSoup(item.get('field_name', ''), 'html.parser')
            metric = soup_name.text.strip()

            # Extract ticker, frequency, and statement from popup_icon
            soup_popup = BeautifulSoup(item.get('popup_icon', ''), 'html.parser')
...
df_wide = df.pivot_table(
    index=['ticker', 'date'],
    values=[col for col in df.columns if col not in ['ticker', 'date', 'frequency', 'statement']],
    aggfunc='first'
).reset_index()
